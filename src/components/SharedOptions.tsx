...
export function SharedOptions({ value, onChange, open, onToggle }: { value: CommonOptions; onChange: (value: CommonOptions) => void; open: boolean; onToggle: () => void }) {
  return (
    <div className="shared-options">
      <button type="button" className="data-button shared-options-button" aria-haspopup="dialog" aria-expanded={open} onClick={onToggle}>공통 작성 옵션 <span className="summary-hint">모든 기능에 적용</span></button>
      {open && <div className="shared-options-body popover-panel" role="dialog" aria-label="공통 작성 옵션">
