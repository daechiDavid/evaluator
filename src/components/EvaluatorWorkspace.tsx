...
export function EvaluatorWorkspace({ initialFeature }: { initialFeature: Feature }) {
  ...
  const renderFeature1 = () => <>
    <div className="feature-intro"><span className="eyebrow">기능 1 · 교과 평가</span><h1>평가계획서의 근거로 교과 문장을 만듭니다.</h1><p>학생 이름 없이 순번만 사용합니다. 문장 생성은 항상 ‘상’ 성취 기준으로 수행되며, 상·중·하 선택은 제공하지 않습니다.</p></div>
  ...
  const renderFeature2 = () => <>
    ...
    <section className="panel"><div className="two-column"><label>필요 학생 수<input type="number" min={1} max={LIMITS.maxStudents} value={state.feature2.studentsCount} onChange={(event) => setState((current) => ({ ...current, feature2: { ...current.feature2, studentsCount: Math.min(LIMITS.maxStudents, Math.max(1, Number(event.target.value) || 1)) } }))} /></label><label>한 문장당 목표 글자 수<input type="number" min={20} max={LIMITS.maxTargetLength} value={state.feature2.targetLength} onChange={(event) => setState((current) => ({ ...current, feature2: { ...current.feature2, targetLength: Math.min(LIMITS.maxTargetLength, Math.max(20, Number(event.target.value) || 20)) } }))} /></label></div><button className="button-secondary" type="button" onClick={() => { const source = feature2Students[0]?.keywords ?? []; setFeature2Students(feature2Students.map((student) => ({ ...student, keywords: [...source] }))); }}>학생 1 키워드를 전체에 복사</button></section>
  ...
  const renderFeature3 = () => <>
    <div className="feature-intro"><span className="eyebrow">기능 3 · 자율활동</span><h1>승인한 활동 주제 네 가지를 문장으로 만듭니다.</h1><p>학생 순번별 주제를 검토하고 승인한 뒤에만 생성합니다. 입력하지 않은 활동과 사건은 추가하지 않습니다.</p></div>
    <section className="panel"><div className="two-column"><label>필요 학생 수<input type="number" min={1} max={LIMITS.maxStudents} value={state.feature3.studentsCount} onChange={(event) => setState((current) => ({ ...current, feature3: { ...current.feature3, studentsCount: Math.min(LIMITS.maxStudents, Math.max(1, Number(event.target.value) || 1)), approved: false } }))} /></label><label>한 문장당 목표 글자 수<input type="number" min={20} max={LIMITS.maxTargetLength} value={state.feature3.targetLength} onChange={(event) => setState((current) => ({ ...current, feature3: { ...current.feature3, targetLength: Math.min(LIMITS.maxTargetLength, Math.max(20, Number(event.target.value) || 20)) } }))} /></label></div><div className="two-column"><label>활동 주제 직접 입력<input placeholder="주제를 입력하고 Enter를 누르세요" onKeyDown={(event) => { if (event.key !== "Enter") return; event.preventDefault(); const value = event.currentTarget.value.trim(); if (value && !state.feature3.topics.includes(value)) { setState((current) => ({ ...current, feature3: { ...current.feature3, topics: [...current.feature3.topics, value], approved: false } })); event.currentTarget.value = ""; } }} /></label><label className="button-secondary file-button">문서에서 주제 추출<input type="file" accept=".pdf,.png,.jpg,.jpeg,.webp,.docx,.pptx,.xlsx" onChange={(event) => void handleActivityExtraction(event.target.files?.[0])} /></label></div><div className="chip-list">{state.feature3.topics.map((topic) => <button type="button" className="chip chip-selected chip-button" key={topic} onClick={() => setState((current) => ({ ...current, feature3: { ...current.feature3, topics: current.feature3.topics.filter((item) => item !== topic), assignments: [], approved: false, results: [] } }))}>{topic} ×</button>)}</div><p className="privacy-note">유효 주제: {state.feature3.topics.length}개 / 최소 4개</p><button type="button" className="button-secondary" onClick={randomizeAssignments} disabled={state.feature3.topics.length < 4}>학생별 네 주제 무작위 배정</button></section>
